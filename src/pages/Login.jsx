...
export default function Login() {
  ...
  useEffect(() => {
    console.log('[Login] Session check. currentUser:', currentUser ? currentUser.email : 'null', 'authLoading:', authLoading);
    if (currentUser && !authLoading) {
      console.log('[Login] User authenticated. Executing auto-redirect to /dashboard');
      navigate('/dashboard');
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('[Login] Firebase login call resolved successfully. Navigating to /dashboard');
      navigate('/dashboard');
    } catch (err) {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#02050f] px-4">
      ...
      >
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="mt-2 text-sm text-gray-400 text-center">
            Step into the future of science education
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center justify-between text-xs">
              <label className="flex items-center text-gray-400 cursor-pointer">
                <input type="checkbox" className="mr-2 rounded border-white/10 bg-white/5 text-brand-purple focus:ring-brand-purple" />
                Remember me
              </label>
              <a href="#" className="text-brand-cyan hover:underline">Forgot password?</a>
            </div>
