...
export const aiService = {
  ...
  async askTutor(context, question, actionType = '') {
    ...
    if (!this.isQuestionRelated(question, filter)) {
      return `I'm currently your tutor for ${filter.lessonName}.

Please ask questions related to this lesson.

Suggested topics:

${filter.allowedTopics.map(t => `• ${t}`).join('\n')}`;
    }

    // 1. If SDK is initialized, run official GoogleGenerativeAI content generation
    if (genAI) {
      try {
        const prompt = `You are an educational science tutor.
Current Subject: ${subject}
Current Category: ${category}
Current Lesson: ${lesson}
Difficulty: ${difficulty}
${selectedPart ? `Selected 3D Structure Focus: ${selectedPart}` : ''}

Answer educationally and accurately. Keep your response concise, engaging, and structured with markdown.
...
* **Other Indicators**:
  ...
  * **Methyl Orange**: Transitions from red (acidic, pH 3.1) to yellow (basic, pH > 4.4).`;
    ...
    return `### 💡 SciLearn AI Tutor:
That is a fascinating question regarding **${title}** (Subject: *${subject}* | Category: *${category}*). 

Under the **${difficulty}** level guidelines, keep in mind that **${firstConcept}** plays a major role: *${firstConceptDesc}*

Let me know if you would like me to:
* Explain the main topic in detail
* Summarize the lesson
* Share some study facts or exam tips`;
  }
};
